...
fn rename_strategy(
    ...
    state: State<AppState>,
    id: u32,
    new_name: String // This is the name Tauri looks for
) -> Result<(), String> {
    let mut data = state.0.lock().unwrap();
...
fn create_asset_group(
    ...
    data.save(&app_handle)?;
    println!("[DEBUG] Создана группа {} и обновлен инвест-план стратегии", name);
    Ok(new_group)
}
...
fn create_asset(
    ...
    if let Some(idx) = existing_index {
        ...
        println!("[DEBUG] Тикер {} уже был в группе — количество объединено: {} шт, средняя цена {}", merged.ticker, merged.amount, merged.buy_price);
        return Ok(merged);
    }
